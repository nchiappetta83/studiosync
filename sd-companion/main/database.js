...
class Database {
  ...
  getProjects() {
    return this.db.prepare('SELECT * FROM projects ORDER BY sort_order, name').all();
  }
  ...
  getPrivateTasks(ownerId) {
    return this.db.prepare('SELECT * FROM private_tasks WHERE owner_id = ? ORDER BY sort_order, created_at').all(ownerId);
  }

  getPrivateTaskById(id) {
    return this.db.prepare('SELECT * FROM private_tasks WHERE id = ?').get(id);
  }

  createPrivateTask(data) {
    const id = data.id || uuidv4();
    const maxOrder = this.db.prepare('SELECT COALESCE(MAX(sort_order), 0) as m FROM private_tasks WHERE owner_id = ?').get(data.owner_id).m;
    this.db.prepare(`
      INSERT INTO private_tasks (id, project_id, owner_id, title, notes, priority, due_date, completed, sort_order)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      data.project_id || null,
    ...
    return this.getPrivateTaskById(id);
  }

  updatePrivateTask(data) {
    const fields = [];
    const values = [];
    for (const key of ['project_id', 'title', 'notes', 'priority', 'due_date', 'completed', 'sort_order']) {
      if (data[key] !== undefined) {
        fields.push(`${key} = ?`);
        values.push(data[key]);
      }
    }
    if (fields.length === 0) return this.getPrivateTaskById(data.id);
    values.push(data.id);
    this.db.prepare(`UPDATE private_tasks SET ${fields.join(', ')} WHERE id = ?`).run(...values);
    return this.getPrivateTaskById(data.id);
  }

  deletePrivateTask(id) {
    this.db.prepare('DELETE FROM private_tasks WHERE id = ?').run(id);
  }
